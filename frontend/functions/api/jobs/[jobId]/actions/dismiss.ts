import { dismissJob } from "../../../../_lib/jobs";
...
export const onRequest = async (context: PagesContext<{ jobId?: string }>) => {
  ...
  try {
    ...
    const payload = await readJson<{ reason: string }>(context.request);
    return jsonResponse(context.env, context.request, await dismissJob(context.env, jobId, payload.reason));
  } catch (error) {
